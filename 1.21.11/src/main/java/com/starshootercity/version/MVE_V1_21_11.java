...
public class MVE_V1_21_11 extends MultiVersionExecutor {


    @Override
    public boolean isCustomAttribute(AttributeModifier modifier) {
        return !modifier.getKey().getNamespace().equals("minecraft");
    }
    ...
    public void sendResourcePacks(Player player, String pack, Map<?, ResourcePackInfo> extraPacks) {
        try {
            ...
            List<net.kyori.adventure.resource.ResourcePackInfo> packs = new ArrayList<>();
            packs.add(packInfo);
            for (ResourcePackInfo resourcePackInfo : extraPacks.values()) {
                if (resourcePackInfo.packInfo() instanceof net.kyori.adventure.resource.ResourcePackInfo info) {
                    packs.add(info);
                }
            }
    ...
    private final net.minecraft.world.item.ItemStack boneMeal = CraftItemStack.asNMSCopy(ItemStack.of(Material.BONE_MEAL));
    ...
    public void dealThornsDamage(Entity target, int amount, Entity attacker) {
        ...
        entity.hurtServer(entity.level().getMinecraftWorld(), entity.damageSources().thorns(((CraftEntity) attacker).getHandle()), amount);
    }
